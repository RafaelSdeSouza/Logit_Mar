...
gRx_S<-read.table("..//data/gRxS_minus.dat",header=TRUE)
gRx_E<-read.table("..//data/gRxE_minus.dat",header=TRUE)
...
dataE_R<-dataE[which(dataE$logM200_L<=(MmeanE-MsdE)+MsdE & dataE$logM200_L >= (MmeanE-MsdE)-MsdE),]
dataS_R<-dataS[which(dataS$logM200_L<=(MmeanS-MsdS)+MsdS & dataS$logM200_L >= (MmeanS - MsdS)-MsdS),]
...
#PRx<-
  ...
  xlab(expression(R/r[200]))+ylab(expression(f[Seyfert]))+coord_cartesian(ylim=c(0.1,1))+
     annotate("text", x = 1, y = 0.9, label =paste("~","log~M[halo]%~~%13.5"),size = 10,parse=TRUE)
...
quartz.save(type = 'pdf', file = '..//figures/P_Rx_minus.pdf',width = 9.5, height = 9)
